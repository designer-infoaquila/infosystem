...
var KTRomaneioAdd = (function () {
			...
			if (validator) {
				...
				validator.validate().then(function (status) {
					if (status == 'Valid') {
						...
						//Form data
						var form_data = $('#romaneio_add_form').serializeArray();
						...
						$.ajax({
							...
							success: function (data, status) {
								...
								if (resposta[0] == 'success') {
									Swal.fire({
										title: 'Inserido!',
										text: 'Romaneio inserido com sucesso',
										icon: 'success',
										timer: 1000,
										onOpen: function () {
											Swal.showLoading();
							...
							error: function (xhr, desc, err) {
								Swal.fire({
									html: 'Romaneio não foi inserido!',
									icon: 'error',
									buttonsStyling: false,
									confirmButtonText: 'OK, entendi!',
									customClass: {
	...
	return {
		// public functions
		init: function () {
			_wizardEl = KTUtil.getById('romaneio_add');
			_formEl = KTUtil.getById('romaneio_add_form');
...
jQuery(document).ready(function () {
	...
	$(document).on('click', '.incluir-produtos', function (e) {
		...
		$.ajax({
			...
			success: function (data) {
				...
				// $('input[name=produtoE]').val(resposta[1]);
				// $('input[name=custoE]').val(numeroParaMoeda(resposta[2]));
				// $('select[name=unidadeE]').val(resposta[3]);
				// $('input[name=espessuraE]').val(numeroParaMoeda(resposta[4]));
			},
		});
	});
	...
	// 		dataType: 'html',
	// 		data: {
	// 			type: 'romaneio_load_edit',
	// 			id: id_produto,
	// 		},
	// 		success: function (data) {
	// 			var resposta = data.split('|');

	// 			$('input[name=produtoE]').val(resposta[0]);
	// 			$('input[name=espessuraE]').val(Number(resposta[1]).toFixed(2).replace('.', ','));
	// 			$('input[name=chapasE]').val(resposta[2]);
	// 			$('select[name=unidadeE]').val(resposta[3]);
	// 			$('input[name=comprimentoE]').val(Number(resposta[4]).toFixed(2).replace('.', ','));
	// 			$('input[name=alturaE]').val(Number(resposta[5]).toFixed(2).replace('.', ','));
	// 			$('input[name=metroE]').val(Number(resposta[6]).toFixed(3).replace('.', ','));
	// 			$('select[name=moedaE]').val(resposta[7]);
	// 			$('input[name=custoE]').val(Number(resposta[8]).toFixed(2).replace('.', ','));
	// 			$('input[name=id_produtoE]').val(resposta[9]);
	// 			$('input[name=id_romaneioE]').val(resposta[10]);
	// 		},
	// 	});
	// });

	// $(document).on('focusout', 'input[name=alturaE]', function (e) {

	// 	var chapas = $('input[name=chapasE]').val();
	// 	var comprimento = moedaParaNumero($('input[name=comprimentoE]').val());
	// 	var altura = moedaParaNumero($('input[name=alturaE]').val());
	...
	// 		$('input[name=metroE]').val(totalM2.toFixed(3).replaceAll('.', ','));
	// 	}
	// });
});
