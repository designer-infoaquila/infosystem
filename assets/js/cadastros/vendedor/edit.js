...
var KTVendedorEdit = (function () {
			...
			if (validator) {
				validator.validate().then(function (status) {
					if (status == 'Valid') {
						...
						//Form data
						var form_data = $('#vendedor_edit_form').serializeArray();
						...
						$.ajax({
							...
							error: function (xhr, desc, err) {
								Swal.fire({
									html: 'Vendedor não foi atualizado!',
									icon: 'error',
									buttonsStyling: false,
									confirmButtonText: 'OK, entendi!',
									customClass: {
	...
	return {
		// public functions
		init: function () {
			_wizardEl = KTUtil.getById('vendedor_edit');
			_formEl = KTUtil.getById('vendedor_edit_form');
