...
// Class definition
var KTRamoDeAtividadeAdd = (function () {
	// Base elements
	var _wizardEl;
	var _formEl;
	var _wizardObj;
			...
			if (validator) {
				...
				validator.validate().then(function (status) {
					if (status == 'Valid') {
						...
						//Form data
						var form_data = $('#ramo_de_atividade_add_form').serializeArray();
						...
						$.ajax({
							...
							success: function (data, status) {
								...
								if (resposta[0] == 'success') {
									Swal.fire({
										title: 'Inserido!',
										text: 'Ramo de Atividade inserido com sucesso',
										icon: 'success',
										timer: 1000,
										onOpen: function () {
											Swal.showLoading();
							...
							error: function (xhr, desc, err) {
								Swal.fire({
									html: 'Ramo de Atividade não foi inserido!',
									icon: 'error',
									buttonsStyling: false,
									confirmButtonText: 'OK, entendi!',
									customClass: {
	...
	return {
		// public functions
		init: function () {
			_wizardEl = KTUtil.getById('ramo_de_atividade_add');
			_formEl = KTUtil.getById('ramo_de_atividade_add_form');
