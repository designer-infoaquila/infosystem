...
var KTEdit = (function () {
			...
			if (validator) {
				validator.validate().then(function (status) {
					if (status == 'Valid') {
						...
						//Form data
						var form_data = $('#moeda_edit_form').serializeArray();
						...
						$.ajax({
							...
							success: function (data, status) {
								...
								if (resposta[0] == 'success') {
									Swal.fire({
										title: 'Atualizado!',
										text: 'Moeda atualizada com sucesso!',
										icon: 'success',
										timer: 1000,
										onOpen: function () {
											Swal.showLoading();
										...
										window.location.href = HOST_URL + 'cadastros/moedas';
									});
								} else {
									Swal.fire({
										html: resposta[1],
	...
	return {
		// public functions
		init: function () {
			_wizardEl = KTUtil.getById('moeda_edit');
			_formEl = KTUtil.getById('moeda_edit_form');
