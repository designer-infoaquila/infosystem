...
// Class definition
var KTMotivoDeCancelamentoEdit = (function () {
	// Base elements
	var _wizardEl;
	var _formEl;
	var _wizardObj;
			...
			if (validator) {
				validator.validate().then(function (status) {
					if (status == 'Valid') {
						...
						//Form data
						var form_data = $('#motivo_de_cancelamento_edit_form').serializeArray();
						...
						$.ajax({
							...
							error: function (xhr, desc, err) {
								Swal.fire({
									html: 'Motivo de Cancelamento não foi atualizado!',
									icon: 'error',
									buttonsStyling: false,
									confirmButtonText: 'OK, entendi!',
									customClass: {
	...
	return {
		// public functions
		init: function () {
			_wizardEl = KTUtil.getById('motivo_de_cancelamento_edit');
			_formEl = KTUtil.getById('motivo_de_cancelamento_edit_form');
...
jQuery(document).ready(function () {
	KTMotivoDeCancelamentoEdit.init();
