...
var KTOrcamentoAdd = (function () {
			...
			if (validator) {
				...
				validator.validate().then(function (status) {
					if (status == 'Valid') {
						...
						//Form data
						var form_data = $('#orcamento_add_form').serializeArray();
	...
	return {
		// public functions
		init: function () {
			_wizardEl = KTUtil.getById('orcamento_add');
			_formEl = KTUtil.getById('orcamento_add_form');
...
jQuery(document).ready(function () {
	...
	$(document).on('click', '.zerar', function (e) {
		e.preventDefault();
		$('.datatable-input-materiais').val('');
		$('#kt_datatable_materiais').DataTable().table().draw();
	...
	$(document).on('click', '.send-vendedor', function (e) {
		e.preventDefault();
		var nome = $('#nome_vendedor').val();

		if (nome == "") {
			Swal.fire({
				html: 'Preencha o nome do Vendedor',
				icon: 'error',
				buttonsStyling: false,
				confirmButtonText: 'OK, entendi!',
				customClass: {
					...
					$.ajax({
						...
						success: function (data, status) {
							...
							if (resposta[0] == 'success') {
								Swal.fire({
									title: 'Adicionado!',
									text: 'Vendedor adicionado com sucesso!',
									icon: 'success',
									timer: 1000,
									onOpen: function () {
										Swal.showLoading();
	...
	$(document).on('click', '.send-cliente', function (e) {
		e.preventDefault();
		var nome = $('#nome_cliente').val();

		if (nome == "") {
			Swal.fire({
				html: 'Preencha o nome do Cliente',
				icon: 'error',
				buttonsStyling: false,
				confirmButtonText: 'OK, entendi!',
				customClass: {
