...
// DB table to use
$table = array(
    array('table' => 'romaneio', 'as' => 't1'),
    array('table' => 'fornecedor', 'as' => 't2', 'join_type' => 'INNER', 'join_on' => 't1.id_fornecedor = t2.id_fornecedor'),

);
// Table's primary key
$primaryKey = 'id_romaneio';

$columns = array(

    array('db' => 'entrada', 'dt' => 0, 'formatter' => function ($d, $row) {
        return $d;
    }),

    array('db' => 'nota_fiscal',  'dt' => 1, 'formatter' => function ($d, $row) {
        return $d;
    }),

    array('db' => 'dt_emissao',  'dt' => 2, 'formatter' => function ($d, $row) {
        if ($d != '00000-00-00') {
    ...
    array('db' => 'nome',  'dt' => 3, 'formatter' => function ($d, $row) {
        return $d;
    }),

    array('db' => 'id_romaneio', 'dt' => 4, 'formatter' => function ($d, $row) {
        return $d;
    }),
