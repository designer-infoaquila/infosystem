...
$columns = array('codigo', 'pessoa', 'nome', 'nomes', 'documento', 'documentos', 'email', 'telefone', 'celular', 'cep', 'endereco', 'numero', 'bairro', 'cidade', 'estado', 'id_empresa');
...
$columns_ = array('codigo', 'pessoa', 'nome', 'nomes', 'documento', 'documentos', 'email', 'telefone', 'celular', 'cep', 'endereco', 'numero', 'bairro', 'cidade', 'estado',);
...
/* Begin::Adicionar */
if ($type == 'transportadora_add') {

    try {
        $sql = 'INSERT INTO transportadora (' . $column_insert . ') VALUES(' . $param_insert . ')';
        ...
        $escreve = fwrite($fp, $number_error . "|transportadora_add|" . debugPDO($sql, $parameters) . "\n\n");
        fclose($fp);
...
/* Begin::Editar */
if ($type == 'transportadora_edit') {
    $cliente_error = 0;

    try {
        $sql = 'UPDATE transportadora SET ' . $param_update . ' WHERE id_transportadora = ' . $_POST['id'];
        $stmt = $pdo->prepare($sql);
        $parameters = array(
            ':codigo' => filter_input(INPUT_POST, "codigo", FILTER_DEFAULT),
            ':pessoa' => filter_input(INPUT_POST, "pessoa", FILTER_DEFAULT),
        ...
        $fp = fopen("../../report/error_log_" . date('dmY') . ".txt", "a");
        $escreve = fwrite($fp, $number_error . "|transportadora_edit|" . debugPDO($sql, $parameters) . "\n\n");
        fclose($fp);
...
/* Begin::Remover */
if ($type == 'transportadora_remove') {

    try {
        $stmt = $pdo->prepare('DELETE FROM transportadora WHERE id_transportadora = :id');
        ...
        $fp = fopen("../../report/error_log_" . date('dmY') . ".txt", "a");
        $escreve = fwrite($fp, $number_error . "|transportadora_delete|DELETE FROM transportadora WHERE id_transportadora = " . $_POST['id'] . "\n\n");
        fclose($fp);
