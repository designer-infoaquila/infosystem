...
$path = '../../assets/media/company/logo-fusion-lastras-alta-transparente_branco.png';
...
if ($totalLinhas >= 1) {
    while ($linhaProd = $consultaProd->fetch(PDO::FETCH_ASSOC)) {
        ...
        $loopProdArray[$linhaProd['material'] . "_" . $linhaProd['ambiente']] .= '<tr>
                        ...
                        <td class="td-center" style="width:80px;">' . number_format($material, 2, ",", ".") . '</td>
                    </tr>';

        $subtotal[$linhaProd['material'] . "_" . $linhaProd['ambiente']] += $vl_total;
        $total += $vl_total;
        $subtotal_material[$linhaProd['material'] . "_" . $linhaProd['ambiente']] += $material;
        ...
        foreach ($arrayAreaUniq as $value) {
            ...
            $loopProd .= '<tr>
                        ...
                        <tr style="background-color:#D9D9D9;">
                            ...
                            <td class="td-center">' . number_format($subtotal_material[$value], 2, ",", ".") . '</td>
                        </tr>';
...
if ($linha['acabamento'] != "" || $linha['acabamento'] != "null") {
    $titleAcabamento = "ACABAMENTOS";
    $corAcabamento = ' background-color:#D9D9D9;';
    if (count(json_decode($linha['acabamento'])) != 0) {
        $tdAcabamento .= '<tr>
            <td colspan="7" style="height:7px; ' . $corAcabamento . '" class="td-right">' . $titleAcabamento . '</td>
        </tr>';
    ...
    $headAcabamento = '<tr>
        <td colspan="7" style="height:7px;" class="td-right"></td>
    </tr>';
} else {
    $titleAcabamento = "";
    $corAcabamento = "";
...
$total_total = $total - $linha['desconto'] + $linha['frete'] + $linha['custos'];
...
<table cellpadding="0" cellspacing="0" border="0" style="width:100%">
    <tr>
        <td rowspan="3" style="width:70%; padding: 0px; margin:0px; background-color:#000000" align="center" >
            <img src="' . $base64 . '" style="width:276px;height:132px; position:relative; left:0px;" />
        </td>
...
<table cellpadding="0" cellspacing="0" border="0" style="width:100%; margin-top:5px;">
    ...
    <tr>
        ...
        <td class="td-center">&nbsp;&nbsp;' . number_format($total_material, 2, ",", ".") . '</td>
    </tr>
    <tr >
        <td colspan="7" style="height:7px;"></td>
    </tr>
    <tr >
        <td colspan="7" class="td-left" style="height:30px;">
        PRAZO DE ENTREGA: ' . $linha['prazo'] . ' dias úteis após projeto de corte aprovado<br>
        ' . $condicao . '
        </td>
    </tr>
    <tr >
        <td colspan="7" class="td-left" style="color:#de6565; font-size:10px;">
            <b>
